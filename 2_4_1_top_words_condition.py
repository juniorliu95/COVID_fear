...
def plot_top_words(top_feat, weights, topic_keys, topic_names, title, save_name, result_path, top_num=50, n_components=3, en=False, trans=None):
    ...
    if en:
        ...
        df.to_csv(os.path.join(root_dir, 'data', 'top_words_conditions_trans.csv'))
...
if not os.path.exists(os.path.join(result_path, title + '_top_words_condition.pkl')):
    for i in range(len(state_list)):
        ...
        vectorizer = CountVectorizer(ngram_range=(1, 1)).fit(text_list)
    ...
    with open(os.path.join(result_path, title + '_top_words_condition.pkl'),'wb') as f:
...
label_trans = ["infection", "Japan", "reserve", "Pfizer", "venue", "mutation", "above", "effect", "side effect", "status"]
...
#%%
df_trans = None
if os.path.exists(os.path.join(root_dir, 'data', 'top_words_conditions_trans.csv')):
    df_trans = pd.read_csv(os.path.join(root_dir, 'data', 'top_words_conditions_trans.csv'))

    df_trans_sort = df_trans.iloc[:,1:3]
    df_trans_sort = df_trans_sort.rename(columns={'before jp':'jp', 'before en':'en'})
